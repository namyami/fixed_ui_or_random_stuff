...
do
    ...
    local function Font_new(name, weight: Enum.FontWeight, data): Font --> ik it looks retarded but less lines
        ...
        if (not makefolder and not isfolder) then return end
        if (not writefile and not isfile) then return end
        if (not getcustomasset) then return end
        if not isfolder('fonts') then makefolder('fonts') end
        if not isfile(`{path}.ttf`) then writefile(`{path}.ttf`, data) end
        local json = { ['name'] = name; ['faces'] = { { ['name'] = 'Regular'; ['weight'] = 400; ['style'] = 'normal'; ['assetId'] = getcustomasset(`{path}.ttf`) } } }
        if not isfile(`{path}.json`) then writefile(`{path}.json`, http:JSONEncode(json)) end
        return Font.new( getcustomasset(`{path}.json`), weight )
    end

    Library.UIFont = Font_new('Plex', Enum.FontWeight.Regular, game:HttpGet('https://github.com/ipufo1/deez-nuts/blob/main/fonts/Plex.txt?raw=true'))
	...
	do
		...
		function Library:SetOpen(bool)
			if typeof(bool) == 'boolean' then
				...
				task.spawn(function()
					game:GetService("TweenService"):Create(Library.PageHolder, TweenInfo.new(0.25, Enum.EasingStyle.Quad, bool and Enum.EasingDirection.Out or Enum.EasingDirection.In), {Position = bool and UDim2.new(0,self.Title.TextBounds.X + 16,0,0) or UDim2.new(0,0,0,0)}):Play()
					--
					if bool then
						task.wait(0.05)
					end
	...
	do
		...
		function Library:Window(Options)
			...
			--
			Pages.Name = "Pages"
			Pages.Position = UDim2.new(0,Title.TextBounds.X + 16,0,0)
...
local window = Library:Window({Name = "epsteinhaxx"})
local pages = {}
for index, value in pairs({"rage", "config"}) do
    pages[value] = window:Page({ Name = value })
end

do -- rage page
    local section = pages["rage"]:Section({Name = "aimbot", LeftTitle =  "general", RightTitle = "settings"}) do
        local toggle = section:Toggle({Name = "enabled"});
        section:Slider({Name = "aimbot fov", Max = 360 - 60 + 50 + 10, Min = 100})
        section:Multibox({Title = "hitboxes", Max = 4,Options = {"head", "torso", "arms", "legs"}})
        section:Button({Name = "idk"})
        section:Textbox({Name = "Niggaaaa", Placeholder = "Niggagaga"})
        toggle:Colorpicker({})
         toggle = section:Toggle({Name = "jEW"});
        toggle:Keybind({})
    end
end
