...
-- Custom Fonts
    ...
    local baseUrl = "https://raw.githubusercontent.com/luminbot/FontStorage/main/"
    local fontdata = {"Proggy"}
    local Fonts, fontindex = { }, 0

    for i, font in fontdata do
        if not isfile(`{font}.ttf`) then
            local success, data = pcall(function() return game:HttpGet(baseUrl .. font .. ".txt") end)
            
            if success and data then
                writefile(`{font}.ttf`, base64_decode(data))
            else
                continue
            end
        end
        ...
        Fonts[font] = fontindex
        -- drawinglib.Fonts[font] = fontindex
        DrawingFontsEnum[fontindex] = Font.new(getcustomasset(`{font}.json`), Enum.FontWeight.Regular)
...
-- Library
    ...
    Library.Font = GetFontFromIndex(0)
    Library.FontSize = 12
    ...
    function Library:CreateSelects(position)
        ...
        -- Keybind Mode
            ...
            local KeybindModeDropdown; KeybindModeDropdown = Library.Dropdown({Holder = KeybindModeDropdownHolder, ContentHolder = Library.Popups}, {Name = "Keybind Mode", Values = {"Toggle", "Hold", "Always", "Single"}, Ignore = true, Callback = function(v)
        ...
        -- Colorpicker
            ...
            Colorpicker.Window.OutlineHolder.Visible = false
            ...
            -- Picker Tab
                ...
                Utility:Connect(ColorpickerSaturationButton.InputBegan, function(input)
                    if input.UserInputType == Enum.UserInputType.MouseButton1 then
                        Colorpicker.SlidingSaturation = true
                        Colorpicker.SlideSaturation(input)
                    end
                end)
                ...
                Utility:Connect(ColorpickerHueButton.InputBegan, function(input)
                    if input.UserInputType == Enum.UserInputType.MouseButton1 then
                ...
                Utility:Connect(ColorpickerAlphaButton.InputBegan, function(input)
                    if input.UserInputType == Enum.UserInputType.MouseButton1 then
    ...
    function Library:MakeDraggable(Holder, Drag)
        local start, objectposition, dragging, currentpos
        local TickCheck = osclock()
        ...
        Drag.InputBegan:Connect(function(input)
            ...
            then
                ...
                objectposition = Holder.AbsolutePosition
            end
        end)

        Utility:Connect(UserInputService.InputChanged, function(input)
            if input.UserInputType == Enum.UserInputType.MouseMovement and dragging then
                local objectSize = Holder.AbsoluteSize

                currentpos = UDim2new(
                    0,
                    mathclamp(objectposition.X + (input.Position.X - start.X), 0, Camera.ViewportSize.X - objectSize.X),
                    0,
                    mathclamp(objectposition.Y + (input.Position.Y - start.Y), 0, Camera.ViewportSize.Y - objectSize.Y)
                )
    ...
    function Library:MakeResizable(Holder, Drag, AllowX)
        AllowX = AllowX == nil and true or false
    ...
    function Library:Window(cfg)
        ...
        Library:MakeDraggable(WindowOutline, TitleBackground)
    ...
    function Library:Dropdown(cfg)
        ...
        Utility:Connect(UserInputService.InputBegan, function(input)
            ...
            if DropdownContentOutline.Visible and not (Utility:MouseOver(DropdownLine, input) or Utility:MouseOver(DropdownContentOutline, input)) then
                Dropdown.Show(false)
            end
        end)
        ...
        function Keybind.Set(input)
            if type(input) == "boolean" then
                ...
                if Keybind.Mode == "Always" then
                    Value = true
                elseif Keybind.Mode == "Single" then
                    task.spawn(function()
                        Keybind.Value = true
            ...
            elseif tostring(input):find("Enum") then 
                ...
                cfg.Callback(Keybind.Value or false)
            elseif tablefind({"Toggle", "Hold", "Always", "Single"}, input) then 
    ...
    function Library:Colorpicker(cfg)
        ...
        Utility:Connect(ColorpickerButton.MouseButton1Down, function()
            ...
            if ColorpickerSelect.Window.OutlineHolder.Visible then
                ...
                ColorpickerSelect.Window.OutlineHolder.Position = UDim2new(0, ColorpickerOutline.AbsolutePosition.x, 0, ColorpickerOutline.AbsolutePosition.y + ColorpickerOutline.AbsoluteSize.y + 2)
        ...
        Utility:Connect(UserInputService.InputBegan, function(input)
            ...
            if Colorpicker.Open and not (Utility:MouseOver(ColorpickerButton, input) or Utility:MouseOver(ColorpickerSelect.Window.OutlineHolder, input)) then
                ColorpickerSelect.Window.OutlineHolder.Visible = false
                Colorpicker.Open = false
            end
        end)
    ...
    function Library:TextTriggers(text)
        ...
        local Triggers = {
            ['{user}'] = Cheat.User,
            ['{name}'] = ('<font color="rgb(%s, %s, %s)">Lumora</font>'):format(mathfloor(AccentTheme.r * 255), mathfloor(AccentTheme.g * 255), mathfloor(AccentTheme.b * 255)),
            ['{hour}'] = os.date("%H"),
            ['{minute}'] = os.date("%M"),
            ['{second}'] = os.date("%S"),
            ['{ap}'] = os.date("%p"),
