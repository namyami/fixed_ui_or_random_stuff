-- jsut a warning this ui sucks ASS!
-- fix everything urself like the mode changer and zindex shit (create another screengui for it)
-- and ismouseoverframe function

-- edit i fixed it.. :middle_finger: though im doing a shitty method fix it urself if u want
...
Library.Sections.__index = Library.Sections
	...
	do
		function Library:NewPicker(name, default, defaultalpha, parent, count, flag, callback)
			...
			ColorOutline.Parent = Library.ScreenGUI2
			...
			local function update()
				local real_pos = players.LocalPlayer:GetMouse()
			...
			Icon.MouseButton1Down:Connect(function()
				...
				ColorOutline.Position = UDim2.new(0, m.X, 0, m.Y)
    ...
    -- theres no functioning watermark so i had to make one
	function Library:Watermark(text)
        text = text or "Nigga"
        local p = Instance.new("GetTextBoundsParams")
        p.Text = text
        p.RichText = true
        p.Font = getfontfromindex(2)
        p.Size = Library.FSize
        p.Width = workspace.CurrentCamera.ViewportSize.X - 32
        local b = game:GetService("TextService"):GetTextBoundsAsync(p)
        local x, y = b.X, b.Y
		...
		Outline.Position = UDim2.new(0, 12, 0, 12)


		Outline.Size = UDim2.new(0, x + 10, 0, y + 6)
		...
		Title.BorderColor3 = Color3.fromRGB(0, 0, 0)
		Title.BorderSizePixel = 0
		Title.Position = UDim2.new(0, 4, 0.5, -y/2.2)
		Title.Size = UDim2.new(1, -8, 1, -6)
		Title.Parent = Inline
        Title.RichText = true
	...
	do
		...
		function Library:Window(Options)
			...
			--
			local ScreenGui = Instance.new("ScreenGui", game:GetService("RunService"):IsStudio() and game:GetService("Players").LocalPlayer.PlayerGui or game.CoreGui)
			ScreenGui.Name = "ScreenGui"
			ScreenGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
			ScreenGui.ResetOnSpawn = false
			--ScreenGui.IgnoreGuiInset = true Fuck u
			Library.ScreenGUI = ScreenGui
			ScreenGui.DisplayOrder = 100

			local ScreenGui2 = Instance.new("ScreenGui", game:GetService("RunService"):IsStudio() and game:GetService("Players").LocalPlayer.PlayerGui or game.CoreGui)
			ScreenGui2.Name = "ScreenGui_Others"
			ScreenGui2.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
			Library.ScreenGUI2 = ScreenGui2
			--ScreenGui2.IgnoreGuiInset = true Fuck u
			...
			Library:Connection(Outline.MouseButton1Down, function()
				local Location = players.LocalPlayer:GetMouse()
		...
		function Sections:Toggle(Properties)
			...
			function Toggle:Keybind(Properties)
				...
				ModeBox.ZIndex = 2
				ModeBox.Parent = Library.ScreenGUI2



				-- ModeBox.Position = KeyFrame.Position
				local UIStroke1 = Instance.new("UIStroke")
				UIStroke1.Name = "UIStroke"
				...
				Library:Connection(KeyFrame.MouseButton2Down, function()
					...
					local m = players.LocalPlayer:GetMouse()
					ModeBox.Position = UDim2.new(0, m.X, 0, m.Y)
...
local window = Library:Window({Name = "NegroHaxx 1337 ◣_◢"})
local tabs = {
    [1] = window:Page({Name = "Ragebaiting Software"}),
    [67] = window:Page({Name = "sudo apt install opsec"})
}
do -- nigga
    local nigger_sectionmeow_pneumonoultramicroscopicsilicovolcanoconiosis = tabs[1]:Section({Name = "NIGGer", AutoSize = true}) do
        local section = nigger_sectionmeow_pneumonoultramicroscopicsilicovolcanoconiosis -- (the name is too fucking long nigga)
        local nigga = section:Toggle({Name = "Nigger_Toggle"})
        nigga:Colorpicker({Name = "Nigga"})
        nigga:Colorpicker({Name = "Nigga"})
        --nigga:Colorpicker({Name = "Nigga"}) ew.. feels like a warcrime.. wtf
        local nigga = section:Toggle({Name = "67"})
        nigga:Keybind({Name = "Niggeerrrsfer"})
        section:List({name = "NIGGER list", Options = {"report", "this", "nigga", "to", "the", "police department."}})
        section:Slider({Name = "Diddy Blud Inches", measurement = " inches", min = 50, max = 4000, decimals = 0.1, Default = 67})
        section:Textbox({Name = "Nigger Textbox", placeholder = "Insert DiddyBlud here"})
        section:Button({Name = "Blud DIddy ahh the nigger", CallBack = function()
            Library:Notification("Diddy the evil nigga ahh 😂", 5, Library.Accent)
        end})
    end
	local section_nigger_this_multi_u_heard, ooo = tabs[1]:MultiSection({Side = "right", Size = 210, Sections = {"1337", "Nigga"}}) do
		local sex = {section_nigger_this_multi_u_heard, ooo}
...
do -- playerlist
	local nigga = tabs[67]:PlayerList({flag = "67_DiddyAhh_Flag😂😂😂😂"}) do
		local section = tabs[67]:Section({Name = "Actions", AutoSize = true})
		section:Toggle({Name = "Rape"})
		section:Toggle({Name = "Ride"})
		section:Toggle({Name = "Make Female"})
		section:Toggle({Name = "Make Gay"})
		section:Toggle({Name = "Make Trans"})
		section:Toggle({Name = "Make Bi"})
		section:Toggle({Name = "Make Lesbian"})
	end
end
Library:Watermark()
window:KeyList()
Library:Notification("Loaded ScaryHaxx 😂😂", 5, Library.Accent)
Library:Notification("Welcome Back Nigga!!", 5, Library.Accent)
return Library
